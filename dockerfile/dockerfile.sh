...
#Dockerfile <== 라는 이름의 파일
# 이미지 생성하는데 필요한 도커 명령어 코드들이 모여있는 설계도 역할의 파일
...
#빌드, 컨테이너 실행, 내부 bash 접속
cd 폴더이동
ls 파일목록 확인


# .dockerignore : 도커 처리시 무시하는 대상 작성
# .gitignore 와 비슷함
...
docker ps -a
#비밀번호같은 중요한 정보는 ENV로 설정을 고정해두는 것보다, 실행 시점에 -e 옵셥으로 지정하는것을 권장
